-- =====================================================
-- DEPLOY MASSIVO - PAINEL ADMINISTRATIVO (CORRIGIDO V2)
-- =====================================================
-- Versão corrigida sem colunas inexistentes
-- =====================================================
...
BEGIN
  ...
  INSERT INTO admin_logs (
    ...
    p_admin_id, COALESCE(v_admin_email, 'unknown'), p_action_type, p_action_description,
...
BEGIN
  ...
  INSERT INTO admin_notifications (
    ...
    p_admin_id, COALESCE(v_admin_email, 'unknown'), p_title, p_message, p_notification_type,
...
-- =====================================================
-- PARTE 5: CRIAR VIEWS (SEM COLUNAS INEXISTENTES)
-- =====================================================

-- View: admin_dashboard_stats (CORRIGIDA - sem last_seen_at)
...
SELECT
  ...
  (SELECT COUNT(*) FROM auth.users WHERE last_sign_in_at >= NOW() - INTERVAL '1 day') as users_active_today,
...
-- View: admin_user_profile (CORRIGIDA - sem last_seen_at, avatar_url, bio, current_theme)
...
-- View: admin_analytics_summary (CORRIGIDA - sem last_seen_at)
...
SELECT
  ...
  (SELECT COUNT(*) FROM auth.users WHERE last_sign_in_at >= NOW() - INTERVAL '1 day') as active_today,
...
-- View: admin_analytics_top_themes (CORRIGIDA - sem current_theme)
CREATE OR REPLACE VIEW admin_analytics_top_themes AS
SELECT
  t.theme_name,
  t.theme_key,
  COUNT(DISTINCT ut.user_id) as users_using
FROM themes t
LEFT JOIN user_themes ut ON ut.theme_id = t.id AND ut.is_active = true
