...
import { UserPlus, Lock, Heart, Send, Users, Flag, MoreVertical } from "lucide-react";
...
const UserProfile = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-hero">
      ...
      <Header />
      <main
        className={`container py-0 sm:py-8 max-w-2xl px-0 sm:px-4 transition-all duration-500 ease-out ${
          isContentVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        }`}
      >
        {/* Cartão de identidade — MESMO componente usado em /profile.
            Antes esta tela tinha uma cópia própria do cabeçalho, então toda
            melhoria feita no perfil (moldura, nível, XP, selos, estatísticas)
            não chegava aqui e o perfil parecia "uma versão antiga". Fonte
            única: mexeu num lugar, muda nos dois. */}
        <ProfilePublicView
          profile={{
            ...
            profile_quote: (profile as any).profile_quote || "",
            church_role: (profile as any).church_role,
            ministries: (profile as any).ministries,
          }}
          badges={badges}
          isOwner={isOwner}
          userId={userId || ""}
        ...
        {/* Ações de quem está visitando (não aparecem no próprio perfil) */}
        {!isOwner && (
          <Card className="mt-4 rounded-none sm:rounded-lg">
            <CardContent className="py-4 flex items-center justify-center gap-2 flex-wrap">
              ...
              {canView && requestStatus === "pending" && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 py-2.5 px-4 rounded-lg">
              ...
              {!canView && (
                <p className="text-sm text-muted-foreground text-center">
                  ...
                  Perfil privado — envie um pedido de amizade para ver o conteúdo completo.
                </p>
              )}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => setShowReportModal(true)} className="text-orange-600">
        ...
        {canView && (
          <div className="space-y-6 mt-4 px-4 sm:px-0">
            ...
            {/* Enviar Palavra de Fé (só entre amigos) */}
            {isFriend && !isOwner && (
              <div className="space-y-3">
                <h3 className="font-semibold flex items-center gap-2">
                ...
                <Textarea
                  ...
                  rows={3}
                />
                <Button onClick={sendFaithPost} disabled={!faithMessage.trim()} className="w-full gap-2">
                  ...
                  <Card key={post.id}>
                    <CardContent className="pt-4">
                      <div className="flex items-start gap-3">
                        <UserAvatar src={post.profiles.avatar_url} fallback={post.profiles.full_name} size="sm" />
            ...
            {userId && <ProfilePhotos userId={userId} isOwner={isOwner} isFriend={isFriend} />}
            {userId && <ProfileVideos userId={userId} isOwner={isOwner} isFriend={isFriend} />}
            {userId && (
              <FriendTestimonials
                profileId={userId}
